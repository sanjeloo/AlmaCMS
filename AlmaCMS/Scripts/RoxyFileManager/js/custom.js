...
function FileSelected(file) {
    ...
    var ImageID = RoxyUtils.GetUrlParam('ImageID');
    var PanelID = RoxyUtils.GetUrlParam('PanelID');
    $(window.parent.document).find('#' + fieldId).attr('value', file.fullPath);
    $(window.parent.document).find('#' + fieldId + 'Label').text(file.fullPath);
    $(window.parent.document).find('#' + ImageID).attr('Src', file.fullPath);
   
    //$(window.parent.document).find('#' + PanelID).dialog('close');
    window.parent.closeCustomRoxy2(PanelID);
  
    //window.parent.CloseRoxyFileUpload();

    //window.parent.CloseVodeoFileUpload();
